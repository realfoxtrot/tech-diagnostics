...
| `/support` | `app/support/page.tsx` + `components/SupportRedirectButton.tsx` | «Запрос в службу поддержки ASUS»: обращение оформляется на официальном сайте ASUS, кнопка «Подтверждаю — перейти на сайт ASUS» |
| `/centers` | `app/centers/page.tsx` | Серверная страница: список активных сервисных центров из БД, карта, список СЦ по городам (заголовок города + карточки; Москва/Санкт-Петербург первыми, остальные по алфавиту) |
| `/ticket` | `app/ticket/page.tsx` | Страница по номеру обращения (?n=): история диагностики (техдокумент), диагноз, привязанный СЦ + PDF-экспорт (кнопка «Сохранить PDF», нативная печать, ?print=1 — автозапуск) |
...
| `SiteHeader.tsx` | Общая шапка: логотип, 4 пункта навигации (гарантийность/техподдержка/диагностика/СЦ), телефон техподдержки 8 800 100-27-87 (desktop lg+, мобильное меню), ThemeToggle, мобильное меню |
