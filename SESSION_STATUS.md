...
2. `8310d1a`-правка — лендинг, статблок сети: вместо «24/7 техподдержка» — «Комманда профессионалов в службе поддержки тел. 8 800 100-27-87, Запрос» (слово «Запрос» → /support).
...
3. `680a1a3` — `/support` переделан: уведомление о переадресации, форма-обёртка удалена (`SupportWrapper`, `api/support/request`), таблица `support_requests` удалена (миграция `0008`), `/privacy` переписан.
4. `37d9cb3` — названия цепочек рекомендаций стали краткими («Проверка зарядки», «Чистка охлаждения»…), 53 шт.
5. `1eb1e43` — **краткое название у каждого шага**: `resolution_steps.title` (миграция `0009`), 126 шагов. Чат: бейдж «Шаг: <название шага>» (меняется с каждым шагом); карта: «Шаг: …» и «Ответ на «…»: помогло/не помогло» — по названию ШАГА. Транскрипт хранит `stepTitle` (fallback `resolutionTitle` для старых записей); в `diagnosis` JSON сессии тоже есть `stepTitle`.
6. `580d7f7` — `/support`: заголовок «Запрос в службу поддержки ASUS», убраны акценты на ПДн.
7. `27f11ca` — **PDF-экспорт карты диагностики**: `components/TicketPrintButton.tsx` (window.print, без новых зависимостей); автозапуск при `?print=1`; в карточке завершения чата кнопка «Сохранить PDF» → `/ticket?ticket=…&print=1`; `@media print` в `globals.css` (светлая палитра, скрыты шапка/футер/кнопки, блоки шагов не рвутся).
8. `7a099fe`, `c965de5` — лендинг: «диагностика бесплатно» → «интерактивная диагностика» (стат-полоса + карточка гарантий).
9. `76041d8` — с `/ticket` убран переключатель темы.
10. `44a5883` — телефон техподдержки 8 800 100-27-87 (`tel:+78001002787`) в шапке: на desktop (lg+) рядом с ThemeToggle, в мобильном меню — в конце списка.
11. `1aca066` — `/privacy`: новый блок «Контакты» (телефон техподдержки + ссылка на `/support`); обещание «контакт ниже» теперь выполнено.
12. `7884d8e` — карточка «Нужна помощь специалиста» в чате: абзац «Запишите номер обращения…», кнопка «Техническая поддержка» (→ `/support`) в ряду кнопок, ссылка «Позвонить 8 800 100-27-87» под кнопками, «Начать заново» ниже по центру.
13. `958e62c` — лендинг: плитка «Проверка статуса ремонта» (→ `/ticket`) заменена на «Проверка гарантийности» (→ `/warranty`). Внимание: 4-я плитка «Оригинальные запчасти» тоже ведёт на `/warranty` — при необходимости развести.
14. `b7cd9cc` — лендинг, «как работает», шаг 4 «Сервисный центр»: текст про прикрепление карты диагностики к запросу в техподдержку или показ инженеру СЦ.
15. `9549d24` — лендинг, hero: кнопка «Проверить гарантийность» (→ `/warranty`, outline-стиль) слева от «Найти сервисный центр».
