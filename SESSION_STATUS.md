...
- Карта — same-origin: все запросы (pbf/глифы/спрайты/Natural Earth) через прокси-роут `/api/map/[...path]` (LRU-кэш 400 записей + Cache-Control 86400; tilejson `/api/map/planet` переписывает tiles[] на прокси — срез планеты отслеживается автоматически). Внешних запросов 0. `public/map-style.json`: glyphs/sprite/tiles → `/api/map/...`.
