...
18. `/centers` — список СЦ под картой сгруппирован по городам: заголовок города (префикс «г. » убран) + сетка карточек, города по алфавиту (ru-locale, `app/centers/page.tsx` `groupByCity`/`cityHeading`); СЦ без города → группа «Без города».
